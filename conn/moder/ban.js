...
module.exports = {
	...
	async execute(message, args) {
		...
		if(!member) {
			return message.reply({embeds: [{
				"title": "**ВНИМАНИЕ**",
				"description": `<:xx:894975989531172875> **${message.author.username}**, укажите, кого хотите забанить!`,
				"color": "#FF0000"
			}]})
		}
    	...
    	if(!args[1]) {
    		return message.reply({embeds: [{
				"title": "**ВНИМАНИЕ**",
  				"description": `<:xx:894975989531172875> **${message.author.username}**, пожалуйста, укажите причину бана!`,
  				"color": "#FF0000"
			}]})
  		}
    	...
    	.setDescription(`<:__:894975989145288795> Выгнан ${member.user.username} (${member.user.id})`)
    	.setColor("#00FF00")
    	.setFooter(`Выгнал ${message.author.username}`)
    	.setTimestamp()
    	.addField('Причина', `${rs}`)
